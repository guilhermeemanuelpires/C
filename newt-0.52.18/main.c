...
void SaveToXml(){
    ...
    for(int i = 0; i < l; i++){
        ...
        xmlSetProp(cliente, "codigo", list[i].codigo);
        xmlSetProp(cliente, "nome", list[i].nome);
        xmlSetProp(cliente, "cartao", (char *) list[i].cartao);
        // xmlSetProp(cliente, "ativo", (char * ) list[i].ativo);
    }
